...
async function editBook(id, title, author, publisher, year, pages, synopsis) {
  try {
    const response = await instance.put(`/books/${id}`, { title, author, publisher, year, pages, synopsis });
    return response.data;
  } catch (error) {
    throw new Error(error.message || 'Something went wrong');
  }
